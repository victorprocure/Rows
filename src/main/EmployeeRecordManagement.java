...
import main.record.RecordCalculatorVisitor;

import java.util.TreeMap;

/**
 * The Employee main.record management class is used as a data store of all unique employees and the main.record visitor assigned
...
class EmployeeRecordManagement {
     ...
     * If none is found we will add (put) a new main.record for an employee.
     *
    ...
    public IRecordVisitor getVisitor(String employee){
        if (!hasEmployee(employee)) {
            // Create employee main.record as none has been created yet
    ...
    /**
     * As we handle market price records slightly different (they don't fall under our action main.record abstract) we use
     ...
     * <p>
     * Not 100% I like this method being in our main.record management class, but it fits for now.
     */
